...
def db_conn(user, pwd, imgHeadName, para1, para2, para3, para4, para5, para6, para7, para8, para9):
    resultRows = caculateData(para1, para2, para3, para4, para5, para6, para7, para8, para9);
    #print("Para start 1:" + para1 + "/" + para2 + "/" + para3 + "/" + para4 + "/" + para5 + "/" + para6 + "/" + para7 + "/" + para8 + "/" + para9);
    ...
    #顯示三個文字，分別是：
    if resultRows == -1:
        plot.text(60, 900, 'NO. of Data Sets: %s\n\nNO. of Data Points: %s\n\nR Square: %s '%(0, 0, 0),style='italic',bbox={'facecolor':'white', 'alpha':0.5,'pad':8});
        #設定X座標軸
        plot.xlim(0,1200);
        #設定Y座標軸
        plot.ylim(0,1200);
    ...
    #dpi 如果設定為80，則圖片大小會變成512x384
    #dpi 如果設定為70，則圖片大小會變成448x336
    #dpi 如果設定為60，則圖片大小會變成384x288
    plot.savefig(imageSaveDir + '\\' + imgHeadName + '.jpg', dpi=100)

def caculateData(para1, para2, para3, para4, para5, para6, para7, para8, para9):
    #組合連線字串並連結資料庫
    ...
    Args = Args + " AND Years >= " + para8;
    Args = Args + " AND Years <= " + para9;
    
    #將字串轉為整數之後在比較
    if int(para8) != 0 | int(para9) != 0:
        Args = Args + " AND Years IS NOT NULL"
    ...
    print("資料筆數:" + str(len(exdata)));
    
    #當篩選條件篩選出來之資料筆數為0筆時，直接回傳不繼續執行
    if len(exdata) == 0:
        return -1;
...
if __name__== "__main__":
    db_conn(username, password, sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], sys.argv[9], sys.argv[10]);